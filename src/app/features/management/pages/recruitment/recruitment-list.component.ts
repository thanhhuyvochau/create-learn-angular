...
@Component({
    ...
    <div class="page-wrap">
      ...
      <section class="page-header">
        <div>
          ...
          <p class="page-subtitle">Quản lý các vị trí tuyển dụng của học viện.</p>
        </div>
      ...
      <section class="stats-grid">
        ...
        <div class="stat-card">
          <p class="stat-label">Thời gian tuyển TB</p>
          <div class="stat-value-row">
            <span class="stat-value">18n</span>
            <span class="stat-badge-muted">-2n so với TB</span>
          </div>
        </div>
      ...
      <section class="table-card">
        ...
        @else if (error()) {
          <div class="error-message">
            ...
            <span>{{ error() }}</span>
            <button mat-button color="primary" (click)="loadJobs()">Thử lại</button>
          </div>
        }
            ...
            <table class="data-table">
              ...
              <tbody>
                ...
                @for (job of pagedJobs(); track job.id) {
                  <tr class="table-row" [class.table-row--inactive]="!job.isActive">
      ...
      <section class="insight-grid">
        ...
        <div class="insight-card insight-card--momentum">
          <div class="insight-content">
            <h3 class="insight-title">Xu hướng tuyển dụng</h3>
            <p class="insight-body">
              Lượt xem các vị trí của học viện tăng 22% trong tháng này.
              Vị trí "Giảng viên Thuật toán cấp cao" đang được quan tâm nhiều nhất.
            </p>
            <div class="insight-stats">
              <div>
                <p class="insight-stat-label">Lượt xem / tuần</p>
                <p class="insight-stat-value insight-stat-value--primary">12,402</p>
              </div>
              <div class="insight-divider"></div>
              <div>
                <p class="insight-stat-label">Điểm chất lượng TB</p>
                <p class="insight-stat-value insight-stat-value--secondary">8.4/10</p>
              </div>
            </div>
          </div>
          <mat-icon class="insight-bg-icon">trending_up</mat-icon>
        </div>

        <!-- Recent Activity -->
        <div class="insight-card insight-card--activity">
          <h3 class="insight-title insight-title--activity">
            <mat-icon class="activity-bolt">bolt</mat-icon>
            Hoạt động gần đây
          </h3>
          <div class="activity-list">
            <div class="activity-item">
              <span class="activity-dot activity-dot--primary"></span>
              <div>
                <p class="activity-label">Hồ sơ mới được nhận</p>
                <p class="activity-meta">Full Stack Developer • 12 phút trước</p>
              </div>
            </div>
            <div class="activity-item">
              <span class="activity-dot activity-dot--secondary"></span>
              <div>
                <p class="activity-label">Vị trí đã bị ẩn</p>
                <p class="activity-meta">Lead UX Researcher • 2 giờ trước</p>
              </div>
            </div>
            <div class="activity-item">
              <span class="activity-dot activity-dot--primary"></span>
              <div>
                <p class="activity-label">5 buổi phỏng vấn được lên lịch</p>
                <p class="activity-meta">Các vị trí giảng viên • Hôm nay, 9:30 SA</p>
              </div>
            </div>
          </div>
          <button class="btn-audit">Xem nhật ký thay đổi</button>
        </div>
  ...
  styles: [`
    ...
    .stat-card--accent { border-left: 4px solid #00a896; }
    .stat-card--overflow { position: relative; overflow: hidden; }
    .stat-bg-circle {
      position: absolute;
      right: -16px;
      top: -16px;
    ...
    .table-head-row { background: #f5f5f5; }
    .th {
      padding: 12px 24px;
      font-size: 0.75rem;
      font-weight: 500;
    ...
    .table-row--inactive .job-title { opacity: 0.5; }
    .table-row--inactive .dept-text { opacity: 0.5; }
    ...
    .status-dot--active { background: #00a896; }
    ...
    .action-btn--view { color: rgba(0, 0, 0, 0.38); }
    ...
    .page-btn--nav { color: rgba(0, 0, 0, 0.38); }
    ...
    .insight-stat-value--primary { color: #00a896; }
    .insight-stat-value--secondary { color: rgba(0, 0, 0, 0.6); }
    .insight-divider { width: 1px; height: 32px; background: #e0e0e0; }
    ...
    .activity-dot--primary { background: #00a896; }
    .activity-dot--secondary { background: rgba(0, 0, 0, 0.38); }
    .activity-label {
      font-size: 0.875rem;
      font-weight: 500;
      color: rgba(0, 0, 0, 0.87);
    ...
    .btn-audit:hover { background: rgba(0, 168, 150, 0.05); }
...
export class RecruitmentListComponent implements OnInit {
  ...
  paginationStart = computed(() =>
    this.filteredJobs().length === 0 ? 0 : this.pageIndex() * this.pageSize() + 1
  );
  paginationEnd = computed(() =>
    Math.min((this.pageIndex() + 1) * this.pageSize(), this.filteredJobs().length)
  );
  ...
  loadJobs(): void {
    ...
    this.recruitmentApi
      ...
      .subscribe({
        ...
        error: () => {
          this.error.set('Không thể tải danh sách vị trí tuyển dụng. Vui lòng thử lại.');
        },
      });
  }
